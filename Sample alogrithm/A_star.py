...
class MiniMap:
    ...
    def generatePoints(self):
        ...
        points = [[self.Point(x, y, 0, 0, None, PointState.UNUSED.value,\
            self.canva.create_rectangle((x * self.size + 3 , y * self.size + 3), ((x + 1) * self.size + 3, (y + 1) * self.size + 3), fill = PointState.UNUSED.value))\
        ...
        while 1:
            ...
            for i in range(-1, 2):
                for j in range(-1, 2):
                    ...
                    yNew = pMin.y + j
                    #如果这个点越界则跳过
                    if(xNew>=self.col or xNew<0 or yNew>=self.row or yNew<0): continue
                    ...
                    if((xNew, yNew) not in closeset and self.points[xNew][yNew].state != PointState.BARRIER.value):
                        ...
                        #如果不在openset中，则代表这个点第一次被访问，直接将pMin设置为pNew的父节点
                            ...
                            self.canva.itemconfig(pNew.rectangle, fill = PointState.TRAVERSED.value)
                            #将这个点加入openlist
